...
int main()
{
	...
	if (*chunk_begin != (ALIGN(malloc_size + SIZE_T_SIZE) | IN_USE))
	{
		snprintf(error_reason, ERROR_MSG_MAX_LEN, "unexpected small chunk size for address %p (expected: %zu, actual: %zu)\n", ptr_last, (ALIGN(malloc_size + SIZE_T_SIZE) | IN_USE), *chunk_begin);
		fail_test(test_name, error_reason);
	}
	if (prev_chunk_begin != NULL && overlaps(chunk_begin, prev_chunk_begin))
	{
	...
	if (small_unsorted_list_len != MIN_NUMBER_OF_SMALL_CHUNKS_PER_ZONE + 1)
	{
		snprintf(error_reason, ERROR_MSG_MAX_LEN, "unexpected number of free chunks in small unsorted list after %zu small frees (expected: %zu, actual: %zu)\n", MIN_NUMBER_OF_SMALL_CHUNKS_PER_ZONE + 1, MIN_NUMBER_OF_SMALL_CHUNKS_PER_ZONE + 1, small_unsorted_list_len);
		fail_test(test_name, error_reason);
	}
